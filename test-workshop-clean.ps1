# MCP Workshop Test Script
param(
    [string]$TestLevel = "local",
    [string]$FunctionUrl = "http://localhost:7071/api/mcp-server"
)

Write-Host "Testing MCP Workshop - Level: $TestLevel" -ForegroundColor Green
...
# Test 1: Ping
Write-Host "`nTesting MCP Protocol..." -ForegroundColor Cyan
...
Write-Host "`nTesting Markdown Review..." -ForegroundColor Cyan
$markdownParams = @{
...
Write-Host "`nTesting Dependency Check..." -ForegroundColor Cyan
$depParams = @{
...
Write-Host "`nTesting AI Code Review..." -ForegroundColor Cyan
$aiParams = @{
    name = "ai_code_review"
    arguments = @{
...
if ($aiResult) {
    try {
        ...
        $status = if ($analysis.status) { $analysis.status } else { "no-status" }
        ...
        Write-Host "  > Could not parse response structure" -ForegroundColor Yellow
    }
} else {
    Write-Host "[FAIL] AI code review failed" -ForegroundColor Red
}

Write-Host "`nWorkshop test complete!" -ForegroundColor Green
